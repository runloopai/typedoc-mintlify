...
interface Page {
  model?: PageModel | Reflection;
}

/**
 * @deprecated This class is deprecated. Use MintlifyReflectionRenderer instead.
 * This class performs text-based transformations which are error-prone.
 * The new reflection-based renderer generates MDX directly from TypeDoc reflections.
 *
...
export class MintlifyPageRenderer {
  theme: unknown;
  page: Page | MarkdownPageEvent | { model?: PageModel | Reflection };

  constructor(theme: unknown, page: Page | MarkdownPageEvent | { model?: PageModel | Reflection }) {
    this.theme = theme;
    this.page = page;
  }
   ...
   */
  render(template: (page: Page | MarkdownPageEvent) => string): string {
    const content = template(this.page);
   ...
   */
  addFrontmatter(content: string): string {
    const model = this.page.model;
    const name = (model && 'name' in model ? model.name : undefined) || 'Documentation';

    // Create keyword-rich title based on type
    let title = name;
    const kindString = model && 'kindString' in model ? model.kindString : undefined;
    if (kindString === 'Class') {
      title = `${name}`;
    ...
    const comment = model && 'comment' in model ? model.comment : undefined;
    const summary =
      comment && 'summary' in comment && Array.isArray(comment.summary)
...
`;
   ...
   */
  private rewriteForUserFocus(content: string): string {
    ...
    content = content.replace(
      /You can use this (class|function|interface|enum) provides/gm,
      'This $1 provides'
    );
    ...
    content = content.replace(
      /> \*\*(\w+)\*\*: (.+?)\n\n(.+?)(?=\n#### |\n### |\n## |\n# |$)/gs,
      (match: string, name: string, type: string, description: string) => {
   ...
   */
  private transformParameters(content: string): string {
    // Match parameter sections: ###### Parameters followed by bullet points
    const paramSectionPattern =
      /###### Parameters\n\n([\s\S]*?)(?=\n###### |\n##### |\n#### |\n### |\n## |\n# |$)/g;

    return content.replace(paramSectionPattern, (match: string, paramsSection: string) => {
   ...
   */
  private transformReturns(content: string): string {
    // Transform standalone Returns sections to ResponseField
    content = content.replace(
      /###### Returns\n\n`([^`]+)`\n\n([^\n]+(?:\n(?!######|#####|####|###|##|#)[^\n]+)*)/gs,
      (match: string, returnType: string, description: string) => {
   ...
   */
  private transformExamples(content: string): string {
      ...
      for (let g = groups.length - 1; g >= 0; g--) {
        ...
        if (group.length > 1) {
          ...
          for (const block of group) {
            ...
            const title =
              block.title || block.language.charAt(0).toUpperCase() + block.language.slice(1);
   ...
   */
  private transformApiDocs(content: string): string {
    const model = this.page.model;
    const kindString = model && 'kindString' in model ? model.kindString : undefined;
    ...
    if (kindString === 'Function' || kindString === 'Method') {
      ...
      content = content.replace(
        /(###### Example\n\n)(```[\s\S]*?```)/g,
        (match: string, prefix: string, codeBlock: string) => {
          // Check if it looks like an API request
          if (
            codeBlock.includes('fetch') ||
            codeBlock.includes('axios') ||
            codeBlock.includes('request') ||
            codeBlock.includes('http')
          ) {
   ...
   */
  private transformInterfaces(content: string): string {
    ...
    // The retry ResponseField should NOT close before nested properties
    // Match: <ResponseField name="retry"...>\nDescription\n</ResponseField>\n\n###### (nested property)
    ...
    // Match: <ResponseField name="retry"...>...nested properties...##### timeout (next section)
    // First, remove any premature closing tags that appear before nested properties
    content = content.replace(
      /(<ResponseField name="retry" type="object">\n[^\n]+\n\n)\n<\/ResponseField>\s*\n\n(######)/g,
      (/** @type {string} */ match, /** @type {string} */ open, /** @type {string} */ next) => {
        // Remove premature closing - retry should stay open
        return open + next;
      }
    );

    // Now ensure retry is properly closed after all nested properties
    content = content.replace(
      /(<ResponseField name="retry" type="object">\n[^\n]+\n\n)([\s\S]*?)(?=\n##### timeout)/g,
      (/** @type {string} */ match, /** @type {string} */ open, /** @type {string} */ nested) => {
        ...
        // Count nested ResponseField tags (delay, maxAttempts)
        ...
        fixedNested = fixedNested.replace(/<\/ResponseField>\s*$/gm, '');

        // If all nested ResponseFields are closed, close the parent retry
        if (nestedOpens === nestedCloses && nestedOpens > 0) {
          if (!fixedNested.trim().endsWith('</ResponseField>')) {
   ...
   */
  private transformClasses(content: string): string {
    ...
    content = content.replace(
      ...
      (
        ...
        const isInterface =
          beforeMatch.includes('#### Interfaces') ||
          beforeMatch.includes('### RequestOptions') ||
          beforeMatch.includes('### User');
   ...
   */
  private transformEnums(content: string): string {
  ...
  private transformCallouts(content: string): string {
    ...
    content = content.replace(/^> \*\*Note:\*\* (.+)$/gm, '<Note>\n$1\n</Note>');

    // Transform > **Warning:** to <Warning>
    content = content.replace(/^> \*\*Warning:\*\* (.+)$/gm, '<Warning>\n$1\n</Warning>');

    // Transform > **Info:** to <Info>
    content = content.replace(/^> \*\*Info:\*\* (.+)$/gm, '<Info>\n$1\n</Info>');

    // Transform > **Tip:** to <Tip>
    content = content.replace(/^> \*\*Tip:\*\* (.+)$/gm, '<Tip>\n$1\n</Tip>');
   ...
   */
  private beautifyStructure(content: string): string {
    ...
    content = content.replace(
      ...
      (
        ...
        if (opens === closes && opens > 0) {
          ...
          // const totalCloses = closes + 1; // +1 for lastClose (unused for now)
    ...
    content = content.replace(
      ...
      (
        ...
        if (opens > closes) {
          ...
          fixedNested = fixedNested.replace(
            unclosedPattern,
            (m: string, tag: string, desc: string) => {
              return `${tag}\n${desc}\n</ResponseField>\n\n`;
            }
          );
        }
   ...
   */
  private transformFunctionParameters(content: string): string {
    ...
    return content.replace(
      ...
      (/** @type {string} */ match, /** @type {string} */ paramsSection) => {
        ...
        const paramFields: FunctionParam[] = [];
        let i = 0;
        ...
        while (i < paramLines.length) {
          ...
          if (paramMatch) {
            ...
            let description = '';
            if (
              i + 1 < paramLines.length &&
              !paramLines[i + 1].match(/^•/) &&
              !paramLines[i + 1].includes('<ParamField')
            ) {
        ...
        if (paramFields.length > 0) {
          ...
          const existingParamFields =
            paramsSection.match(/<ParamField[\s\S]*?<\/ParamField>/g) || [];
